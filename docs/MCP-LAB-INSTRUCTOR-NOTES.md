...
A small stand-in serving the five designed capture endpoints, so Tiers 2–4 run before capture REST ships. It mirrors the shapes in [wifi-capture.md](https://github.com/WLAN-Pi/wlanpi-core/blob/dev/docs/api/wifi-capture.md) and the [consumer guide](https://github.com/WLAN-Pi/wlanpi-core/blob/dev/docs/api/wifi-capture-consumer-guide.md).
